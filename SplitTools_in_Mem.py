...
def each_sub_proc(file_name, dir_name, dataset_root, image_id, labels, each_image_json):
    ...
    image_info = pycococreatortools.create_image_info(image_id, file_name + ".jpg", instance_image.size)
    ...
    for item in image_label_instance_infomatrix:
        ...
        annotation_info = pycococreatortools.create_annotation_info(
            ...
            instance_image.size, tolerance=2)
    ...
    save_path = "{}/{}/massive_annotations/image{}_info.json".format(dataset_root, dir_name, image_id)
...
def load_datasets_and_proc(dataset_root, dir_name, files):
    ...
    for idx, image_filename in enumerate(files):
        pool.apply_async(each_sub_proc, args=(image_filename, dir_name, dataset_root, idx + 1, labels, each_image_json))
...
def readout_each_image(dataset_root, dir_name, seq):
    json_saved_path = '{}/mapillary/{}/massive_annotations/image{}_info.json'.format(dataset_root, dir_name, seq)
...
def main(dir_name):
    ...
    if os.path.exists("{}/{}/massive_annotations".format(dataset_root, dir_name)) is False:
    ...
    combined_annotations['annotations'] = sorted(combined_annotations['annotations'], key=lambda item: item.__getitem__('image_id'))

    for idx in range(len(combined_annotations['annotations'])):
        combined_annotations['annotations'][idx]['id'] = idx + 1

    combined_json_path = '{}/{}/instances_shape_{}2020.json'.format(dataset_root, dir_name, dir_name)
